...
test_that("binary and categorical are routed by observed level count, as the macro does", {
  d <- data.frame(g = c(1, 1, 0, 0), one = c(1, 1, 1, 1), two = c("m", "f", "f", "f"),
                  three = c("a", "b", "c", "a"))
  obj <- ps_stddiff(d, treatment_col = "g", binary = c("one", "three"), categorical = "two")
